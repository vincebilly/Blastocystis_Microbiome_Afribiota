...
plot_deseq_sig_all <- ggplot(log_melt4_mel, aes(x=variable, y=reorder(Cplt_taxo2,value_in), size= value, color=direction)) +
  geom_point(alpha=0.5) +
  theme_light() +
  scale_color_manual(values=c("orange","blue"), name="Change in abundance") +
...
tax_otu_agg <- aggregate(tax_otu, list(tax_otu$Rank5),max)
row.names(tax_otu_agg) <- tax_otu_agg$Group.1
tax_otu_agg$Group.1 <- NULL
...
ggplot(merge_spl_tax_otu, aes(x=Cq, y=Prevotellaceae)) +
  geom_point(aes(color = Cq)) +
  scale_x_continuous(trans='log10') +
  scale_y_continuous(trans='log10')

ggplot(merge_spl_tax_otu, aes(x=Cq, y=Bacteroidaceae)) +
  geom_point(color="grey30") +
  scale_x_continuous(trans='log10') +
  scale_y_continuous(trans='log10') +
  stat_cor(method="spearman")  +
  ylab("Bacteroidaceae") +
  xlab("Ct value from qPCR (reversed)") +
  scale_x_reverse() +
  theme_classic()
...
tax_otu_agg <- aggregate(tax_otu, list(tax_otu$Rank6),max)
row.names(tax_otu_agg) <- tax_otu_agg$Group.1
tax_otu_agg$Group.1 <- NULL
...
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Prevotella>0)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Prevotellaceae_Un>0)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, 'Prevotellaceae_UCG-003'>0)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Prevotellaceae_NK3B31_group>0)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Prevotella_9>0)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Prevotella_7>0)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Prevotella_6>0)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Prevotella_2>0)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Paraprevotella>0)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Bacteroides>0)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Alloprevotella>0)


merge_spl_tax_otu$Cq <- as.numeric(merge_spl_tax_otu$Cq)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Cq_presence_absence =="Presence_qPCR")

merge_spl_tax_otu <- merge_spl_tax_otu[,c("Cq","Alloprevotella","Bacteroides","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")]
merge_spl_tax_otu_melt <- melt(merge_spl_tax_otu, id.vars="Cq",measure.vars = c("Alloprevotella","Bacteroides","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un"))#[, c("binome","value")]

  
  
ggplot(merge_spl_tax_otu_melt, aes(x=Cq, y=value)) +
  geom_point(aes(color = Cq)) +
  scale_x_continuous(trans='log10') +
  scale_y_continuous(trans='log10') +
  stat_cor(method="spearman") +
...
for(i in 1:length(bacterial_taxa)) {
  interm_table <- merge_spl_tax_otu[,c("Cq",bacterial_taxa[i])]
  colnames(interm_table) <- c("Cq","Bacterial_Taxa")
  interm_table <- subset(interm_table, Bacterial_Taxa>0)
  plot <- ggplot(interm_table, aes(x=Cq, y= Bacterial_Taxa)) +
    geom_point(color="grey30") +
    scale_x_continuous(trans='log10') +
    scale_y_continuous(trans='log10') +
    stat_cor(method="spearman") +
...
plot <- ggarrange(list_scatterplot[[2]],list_scatterplot[[4]],list_scatterplot[[8]], labels = c("A","B","C"), nrow = 1, ncol = 3)
...
##Cq value and divesity of Prevotella.
...
merge_spl_tax_otu <- merge_spl_tax_otu[,c("Cq","Alloprevotella","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")]
merge_spl_tax_otu$Cq <- as.numeric(merge_spl_tax_otu$Cq)
...
ggplot(merge_spl_tax_otu, aes(x=Cq, y= richness)) +
  geom_point(color="grey30") +
  #scale_x_continuous(trans='log10') +
  #scale_y_continuous(trans='log10') +
  stat_cor(method="spearman") +

...
# Comparing data set from with all combination
#########################################
...
pdf("/Users/vincebilly/Desktop/venn.diagram_comparison_dataset.pdf", width = 10)
...
qPCR_Metadata %>%
  group_by(stunted) %>%
  anova_test(Shannon16s_new ~ Cq_presence_absence*pays, error = model)
...
qPCR_Metadata %>%
  group_by(pays) %>%
  emmeans_test(Shannon16s_new ~ Cq_presence_absence+stunted, p.adjust.method = "BH")
...
qPCR_Metadata %>%
  group_by(stunted) %>%
  anova_test(Number_ASVs16s_new ~ Cq_presence_absence*pays, error = model)
...
diet_score <- qPCR_Metadata[,c("diet_score","Sample")]
...
#########################################################################
###      ANCOVA Blastocystis and Number of ASV, stunted, country      ###
#########################################################################

qPCR_Metadata_presence <- subset(qPCR_Metadata, Cq_presence_absence =="Presence_qPCR")

plot <- ggscatter(
  qPCR_Metadata_presence, x = "Cq", y = "Number_ASVs16s_new", add = "reg.line", palette = c("navy","dodgerblue"), facet.by = c("pays","stunted")) +
...
qPCR_Metadata_presence %>% anova_test(Number_ASVs16s_new ~ stunted*Cq)

# Fit the model, the covariate goes first
model <- lm(Number_ASVs16s_new ~ Cq + stunted, data = qPCR_Metadata_presence)
...
model.metrics %>% levene_test(.resid ~ stunted)

model.metrics %>% 
  filter(abs(.std.resid) > 3) %>%
  as.data.frame()

res.aov <- qPCR_Metadata_presence %>% anova_test(Number_ASVs16s_new ~ Cq + stunted + pays)
get_anova_table(res.aov)

pwc <- qPCR_Metadata_presence %>% 
  emmeans_test(
    Number_ASVs16s_new ~ stunted, covariate = Cq,
    p.adjust.method = "BH"
  )
pwc
...
tbl1 <- qPCR_Metadata_presence %>%
  group_by(pays) %>%
  anova_test(Number_ASVs16s_new ~ Cq + stunted)
tbl2 <- qPCR_Metadata_presence %>%
  anova_test(Number_ASVs16s_new ~ Cq + stunted + pays)
tbl3 <- qPCR_Metadata_presence %>%
  group_by(stunted) %>%
  anova_test(Number_ASVs16s_new ~ Cq + pays)
tbl4 <- qPCR_Metadata_presence %>%
  group_by(stunted,pays) %>%
  anova_test(Number_ASVs16s_new ~ Cq)

ggscatter(qPCR_Metadata_presence, x = "Cq", y = "Number_ASVs16s_new",
          facet.by  = c("pays", "stunted"), 
...
qPCR_Metadata_presence %>%
  ...
  anova_test(Number_ASVs16s_new ~ group*Cq)

# Fit the model, the covariate goes first
model <- lm(Number_ASVs16s_new ~ Cq + stunted*pays, data = qPCR_Metadata_presence)
# Inspect the model diagnostic metrics
model.metrics <- augment(model) %>%
  dplyr::select(-.hat, -.sigma, -.fitted) # Remove details
head(model.metrics, 3)
...
qPCR_Metadata_presence %>% 
  group_by(stunted) %>%
  emmeans_test(
    Number_ASVs16s_new ~ pays, covariate = Cq,
    p.adjust.method = "BH"
  )


pwc2 <- qPCR_Metadata_presence %>% 
  group_by(stunted) %>%
  emmeans_test(
    Number_ASVs16s_new ~ pays, covariate = Cq,
    p.adjust.method = "BH"
  ) %>%
  dplyr::select(-df, -statistic, -p) # Remove details

# Line plot
lp <- ggline(
  get_emmeans(pwc), x = "pays", y = "emmean", 
  color = "stunted", palette = c("navy","dodgerblue")
) +
  geom_errorbar(
    aes(ymin = conf.low, ymax = conf.high, color = stunted), 
    width = 0.1
  )

# Comparisons between treatment group at each exercise level
pwc <- pwc %>% add_xy_position(x = "pays", fun = "mean_se", step.increase = 0.2)
pwc.filtered <- pwc %>% filter(pays == "high")
lp + 
  stat_pvalue_manual(
...
pwc2 <- pwc2 %>% add_xy_position(x = "pays", fun = "mean_se")
pwc2.filtered <- pwc2 #%>% filter(stunted == "yes")
lp + 
  stat_pvalue_manual(
    pwc2.filtered, hide.ns = TRUE, tip.length = 0,
    step.group.by = "stunted", color = "stunted"
  ) +
  labs(
    subtitle = get_test_label(res.aov,  detailed = TRUE),
    caption = get_pwc_label(pwc2)
...
#########################################################################
###      ANCOVA Blastocystis and Shannon, stunted, country            ###
#########################################################################

qPCR_Metadata_presence <- subset(qPCR_Metadata, Cq_presence_absence =="Presence_qPCR")

plot <- ggscatter(
  qPCR_Metadata_presence, x = "Cq", y = "Shannon16s_new", 
  add = "reg.line", color = "blue", facet.by = c("pays","stunted")) +
...
res.aov <- qPCR_Metadata_presence %>% anova_test(Shannon16s_new ~ Cq + stunted + pays)
get_anova_table(res.aov)

pwc <- qPCR_Metadata_presence %>% 
  emmeans_test(
    Shannon16s_new ~ stunted, covariate = Cq,
    p.adjust.method = "BH"
  )
pwc
...
# Effect of treatment at each level of exercise
tbl1 <- qPCR_Metadata_presence %>%
  anova_test(Shannon16s_new ~ Cq + stunted + pays)
tbl2 <- qPCR_Metadata_presence %>%
  group_by(pays) %>%
  anova_test(Shannon16s_new ~ Cq + stunted)
tbl3 <- qPCR_Metadata_presence %>%
  group_by(stunted) %>%
  anova_test(Shannon16s_new ~ Cq + pays)
tbl4 <- qPCR_Metadata_presence %>%
  group_by(stunted,pays) %>%
  anova_test(Shannon16s_new ~ Cq)
...
qPCR_Metadata_presence %>%
  unite(col = "group", stunted, pays) %>%
  anova_test(Shannon16s_new ~ group*Cq)

# Fit the model, the covariate goes first
model <- lm(Shannon16s_new ~ Cq + stunted*pays, data = qPCR_Metadata_presence)
# Inspect the model diagnostic metrics
model.metrics <- augment(model) %>%
  dplyr::select(-.hat, -.sigma, -.fitted) # Remove details
head(model.metrics, 3)
...
qPCR_Metadata_presence %>% 
  group_by(stunted) %>%
  emmeans_test(
    Shannon16s_new ~ pays, covariate = Cq,
    p.adjust.method = "BH"
  )
...
#Blasto and stunting in MAdagascar
subset_project_data_subset <- subset_samples(subset_project_data, pays =="Madagascar")
project_data.bray <- phyloseq::distance(subset_project_data_subset, method = "bray")
sample_df <- as.data.frame(as.matrix(sample_data(subset_project_data_subset)))
res.adonis.rarefied.bray <- adonis2(project_data.bray ~ stunted*Cq_presence_absence, data =sample_df, method="bray");res.adonis.rarefied.bray
res.adonis.rarefied.bray <- adonis2(project_data.bray ~ Cq_presence_absence*stunted, data =sample_df, method="bray");res.adonis.rarefied.bray

#Blasto and stunting in CAR
subset_project_data_subset <- subset_samples(subset_project_data, pays =="RCA")
project_data.bray <- phyloseq::distance(subset_project_data_subset, method = "bray")
sample_df <- as.data.frame(as.matrix(sample_data(subset_project_data_subset)))
res.adonis.rarefied.bray <- adonis2(project_data.bray ~ stunted*Cq_presence_absence, data =sample_df, method="bray");res.adonis.rarefied.bray
res.adonis.rarefied.bray <- adonis2(project_data.bray ~ Cq_presence_absence*stunted, data =sample_df, method="bray");res.adonis.rarefied.bray
...
stunted_vector <- c("Stunted","Non-Stunted")#,"Both_clinical")
summary_table <- data.frame(Cplt_taxo=c(NA),
                            Rank=c(NA))
summary_correlation_table <- data.frame(Cplt_taxo=c(NA))
for(p in 1:length(pays_vector)){
  ...
  for(s in 1:length(stunted_vector)){
    ...
    ############################################################
    rank_vector <- c("Rank2","Rank3","Rank4","Rank5","Rank6","Rank7")
    for(rnk in 1:length(rank_vector)){
      ...
      res_p_ordered_wtaxa.var3$Rank <- rank_vector[rnk]
      table_to_keep <- res_p_ordered_wtaxa.var3[,c("Cplt_taxo","Rank","log2FoldChange")]
      ...
      tax_otu$Rank <- rank_vector[rnk]
      lefse_table <- rbind(lefse_table, tax_otu)                                     #Concatenate result from each Rank
    }
    ###################################################################
    ###       DESEQ2 formating and combining several dataset        ###
    ###################################################################
    deseq_table$Cplt_taxo <- gsub("[|]NA","",deseq_table$Cplt_taxo)
    summary_table <- merge(summary_table,deseq_table,by=c("Cplt_taxo","Rank"), all=TRUE)
    ...
    lefse_table$Row.names <- paste0(lefse_table$Rank1,"|",lefse_table$Rank2,"|",lefse_table$Rank3,"|",lefse_table$Rank4,"|",lefse_table$Rank5,"|",lefse_table$Rank6,"|",lefse_table$Rank7)
    #Remove NA in the same column
    lefse_table$Row.names <- gsub("[|]NA","",lefse_table$Row.names)
    #Remove all "Rank" columns
    lefse_table <- lefse_table[,c(setdiff(colnames(lefse_table),colnames(tax_table(project_data_2))),"Rank")]
    lefse_table2 <- lefse_table
    ...
    #####################################################################
    ### Correlation between bacterial taxa and Blastocystis abundance ###
    #####################################################################
    subtable <- lefse_table2
    row.names(subtable) <- subtable$Row.names
    subtable$Row.names <- NULL
    subtable <- as.data.frame(t(subtable))
    subtable <- merge(qPCR_Metadata,subtable,by="row.names")
    subtable <- subset(subtable, Cq_presence_absence =="Presence_qPCR")
    taxa_vec <- colnames(subtable)[grep("Archaea|Bacteria",colnames(subtable))]
    for(tax in 1:length(taxa_vec)){
      subtable[,taxa_vec[tax]] <- as.numeric(subtable[,taxa_vec[tax]])
      Spearman <- cor.test(subtable$Cq,subtable[,taxa_vec[tax]], method = "spearman")
      final_table <- data.frame(Cplt_taxo = taxa_vec[tax],
                                Pvalue_Spearman = Spearman$p.value,
                                Rho = Spearman$estimate)
      correlation_table <- rbind(correlation_table,final_table)
    }
    correlation_table$Pvalue_Spearman <- p.adjust(correlation_table$Pvalue_Spearman, method = "BH", n = length(correlation_table$Pvalue_Spearman))
    colnames(correlation_table) <- c("Cplt_taxo",
                                     paste0("Pvalue_Spearman_",pays_vector[p],"_",stunted_vector[s]),
                                     paste0("Rho_",pays_vector[p],"_",stunted_vector[s]))
    summary_correlation_table <- merge(summary_correlation_table,correlation_table,by="Cplt_taxo", all=TRUE)
  }
}
...
####################################################################
###         Create the table with all taxa combination           ###
####################################################################

lefse_table <- data.frame()                                                     #Create data frame
rank_vector <- c("Rank2","Rank3","Rank4","Rank5","Rank6","Rank7")
for(rnk in 1:length(rank_vector)){
  if (rank_vector[rnk]=="Accession"){
    project_data_1_rank <- subset_project_data
  } else {
    project_data_1_rank <- tax_glom(subset_project_data, taxrank=rank_vector[rnk])
  }
  otu <- as.data.frame(t(otu_table(project_data_1_rank)))                       #Extract otu table
  tax <- as.data.frame(as(tax_table(project_data_1_rank), "matrix"))            #Extract tax table
  tax_otu <- merge(tax, otu, by ="row.names", all = T)                          #Merge tax and otu table
...
lefse_table$Cplt_taxo <- paste0(lefse_table$Rank1,"|",lefse_table$Rank2,"|",lefse_table$Rank3,"|",lefse_table$Rank4,"|",lefse_table$Rank5,"|",lefse_table$Rank6,"|",lefse_table$Rank7)
lefse_table$Cplt_taxo <- gsub("[|]NA","",lefse_table$Cplt_taxo)
lefse_table <- lefse_table[,!grepl("Rank[1-7]", colnames(lefse_table))]
lefse_table <- lefse_table[,!grepl("Row.names", colnames(lefse_table))]

rel_abun_tbl_rnk_melt_all <- data.frame()
for(rnk in 1:length(rank_vector)){
  rel_abun_tbl_rnk <- subset(lefse_table, Rank == rank_vector[rnk])
  rel_abun_tbl_rnk[,!grepl("Rank|Cplt_taxo",colnames(rel_abun_tbl_rnk))] <- rel_abun_tbl_rnk[,!grepl("Rank|Cplt_taxo",colnames(rel_abun_tbl_rnk))]/colSums(rel_abun_tbl_rnk[,!grepl("Rank|Cplt_taxo",colnames(rel_abun_tbl_rnk))])
  colSums(rel_abun_tbl_rnk[,!grepl("Rank|Cplt_taxo",colnames(rel_abun_tbl_rnk))])
  rel_abun_tbl_rnk_melt <- melt(rel_abun_tbl_rnk, id.vars=c("Cplt_taxo","Rank"));head(rel_abun_tbl_rnk_melt)
  rel_abun_tbl_rnk_melt_all <- rbind(rel_abun_tbl_rnk_melt_all,rel_abun_tbl_rnk_melt)
}

pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Jitter_rel_abun_allrank.pdf", height = 200, width = 20)
plot_taxa_proportion <- ggplot(rel_abun_tbl_rnk_melt_all, aes(y=Cplt_taxo, x=value)) +
  geom_point() +
  theme_light() +
  facet_grid(rows = vars(Rank), scales = "free", space = "free")
print(plot_taxa_proportion)
dev.off()
         
prop_table <- data.frame()                                                     #Create dataframe
rank_vector <- c("Rank2","Rank3","Rank4","Rank5","Rank6","Rank7")
for(rnk in 1:length(rank_vector)){
  test <- subset(lefse_table, Rank== rank_vector[rnk])
  row.names(test) <- test$Cplt_taxo
  test$Cplt_taxo <- NULL
  test$Rank <- NULL
  Proportion_tbl <- as.data.frame(rowSums(test)/sum(rowSums(test))*100)
  Total_Reads_tbl <- as.data.frame(rowSums(test))
  test[test>0] <- 1
  Prevalence_tbl <- as.data.frame(rowSums(test)/length(colnames(test))*100)
  tbl_plt <- merge(Proportion_tbl,Prevalence_tbl, by="row.names")
  row.names(tbl_plt) <- tbl_plt$Row.names
  tbl_plt$Row.names <- NULL
  tbl_plt <- merge(tbl_plt,Total_Reads_tbl, by="row.names")
  colnames(tbl_plt) <- c("Cplt_taxo","Proportion","Prevalence","Total_Reads_tbl")
  tbl_plt$Rank <- rank_vector[rnk]
  prop_table <- rbind(prop_table, tbl_plt)                                    #Concatenate result from each Rank
}

prop_table$Cplt_taxo <- gsub("[|]Rank.*","",prop_table$Cplt_taxo)
prop_table$Cplt_taxo <- gsub("[|]ASV*","",prop_table$Cplt_taxo)
prop_table$Cplt_taxo <- gsub("[|]Accession*","",prop_table$Cplt_taxo)

pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Scatter_prevalence_proportion.pdf", height = 10, width = 5)
plot <- ggplot(prop_table, aes(y=Prevalence, x=Proportion)) +
  geom_point() +
...
prop_table$order <- prop_table$Proportion
prop_table$Total_Reads_tbl_log <- log(prop_table$Total_Reads_tbl+1,base = 10)
prop_table_melt <- melt(prop_table, id.vars=c("Cplt_taxo","Rank","order"),
                       measure.vars = c("Total_Reads_tbl","Total_Reads_tbl_log","Proportion","Prevalence"));head(prop_table_melt)
prop_table_melt$Rank <- factor(prop_table_melt$Rank, levels=c("Rank2","Rank3","Rank4","Rank5","Rank6","Rank7"))
pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Taxa_proportion.pdf", height = 200, width = 30)
plot_taxa_proportion <- ggplot(prop_table_melt, aes(y=reorder(Cplt_taxo,order), x=value)) +
  geom_bar(stat="identity") +
  theme_light() +
  facet_grid(Rank ~ variable, scales = "free", space = "free_y")
print(plot_taxa_proportion)
dev.off()

prop_table_rel_abun_allrnk <- merge(rel_abun_tbl_rnk_melt_all,prop_table_melt,"Cplt_taxo")
pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Jitter_rel_abun_allrank.pdf", height = 200, width = 20)
plot_taxa_proportion_rel <- ggplot(prop_table_rel_abun_allrnk, aes(y=reorder(Cplt_taxo,order), x=value.x)) +
  geom_point() +
  theme_light() +
  facet_grid(rows = vars(Rank.x), scales = "free", space = "free") +
  theme(axis.text.y = element_blank())
print(plot_taxa_proportion_rel)
dev.off()

pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Jitter_rel_abun_allrank_logscale.pdf", height = 200, width = 20)
plot_taxa_proportion_rel_logscale <- ggplot(prop_table_rel_abun_allrnk, aes(y=reorder(Cplt_taxo,order), x=value.x)) +
  geom_point() +
  scale_x_continuous(trans='log10') +
  theme_light() +
  facet_grid(rows = vars(Rank.x), scales = "free", space = "free") +
  theme(axis.text.y = element_blank())
print(plot_taxa_proportion_rel_logscale)
dev.off()

#Combine plot
plot_combined <- ggarrange(plot_taxa_proportion,plot_taxa_proportion_rel,plot_taxa_proportion_rel_logscale, nrow=1)
ggsave2(plot_combined, filename = "/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Taxa_prop_prev_rel_allrank.pdf", height = 150, width = 100,limitsize = FALSE)

####################################################################

##############################################################################
### Pie chart Proportion of ASV, reads, between enriched and depleted taxa ###
##############################################################################

subset_deseq <- summary_table[,c("Both_Country_Both_clinical","Cplt_taxo")]
#Sanity check
intersect(subset_deseq$Cplt_taxo,prop_table$Cplt_taxo)
setdiff(subset_deseq$Cplt_taxo,prop_table$Cplt_taxo)
setdiff(prop_table$Cplt_taxo,subset_deseq$Cplt_taxo)

#
deseq_for_pie_chart <- merge(subset_deseq,prop_table,by="Cplt_taxo",all=TRUE)
deseq_for_pie_chart <- deseq_for_pie_chart[!is.na(deseq_for_pie_chart$Cplt_taxo),]
deseq_for_pie_chart <- deseq_for_pie_chart%>% mutate(direction= case_when(deseq_for_pie_chart$Both_Country_Both_clinical>0~"Enriched",
                                                   deseq_for_pie_chart$Both_Country_Both_clinical<0~"Depleted",
                                                   is.na(deseq_for_pie_chart$Both_Country_Both_clinical)~"No effect"))
deseq_for_pie_chart_ASV_all<- data.frame()
deseq_for_pie_chart_reads_all<- data.frame()

for(rnk in 1:length(rank_vector)){
  deseq_for_pie_chart_rnk <- subset(deseq_for_pie_chart,Rank==rank_vector[rnk])
  deseq_for_pie_chart_ASV <- as.data.frame(table(deseq_for_pie_chart_rnk$direction))
  colnames(deseq_for_pie_chart_ASV) <- c("direction","Freq")
  deseq_for_pie_chart_ASV$Rank <- rank_vector[rnk]
  deseq_for_pie_chart_ASV$Type <- "ASV"
  deseq_for_pie_chart_ASV_all <- rbind(deseq_for_pie_chart_ASV_all,deseq_for_pie_chart_ASV)
  deseq_for_pie_chart_reads <- deseq_for_pie_chart_rnk[,c("direction","Total_Reads_tbl")]
  deseq_for_pie_chart_reads <- deseq_for_pie_chart_reads[!is.na(deseq_for_pie_chart_reads$Total_Reads_tbl),]
  deseq_for_pie_chart_reads <- deseq_for_pie_chart_reads <- deseq_for_pie_chart_reads %>%                                        
    group_by(direction) %>% 
    summarise_at(vars(Total_Reads_tbl), 
                 list(Freq = sum))
  deseq_for_pie_chart_reads$Rank <- rank_vector[rnk]
  deseq_for_pie_chart_reads$Type <- "Number reads"
  deseq_for_pie_chart_reads_all <- rbind(deseq_for_pie_chart_reads_all,deseq_for_pie_chart_reads)
  
}

final_table <- rbind(deseq_for_pie_chart_ASV_all,deseq_for_pie_chart_reads_all)

cp <- coord_polar(theta = "y")
cp$is_free <- function() TRUE
pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Pie_Deseq_ASV_reads_all_ranks.pdf", height = 3, width = 7)
ggplot(final_table, aes(x="",y = Freq, fill = direction)) + 
  geom_bar(stat="identity") +
  cp+
  scale_fill_manual(values=c("orange", "blue", "gray")) +
  facet_wrap(Type~Rank, scales = "free", nrow = 2) +
  theme_void() +
  theme(legend.position = "none") +
  geom_text(aes(label = Freq),
            position = position_stack(vjust = 0.5))
dev.off()
ggplot(deseq_for_pie_chart_reads_all, aes(x="",y = Number_reads, fill = direction)) + 
  geom_bar(stat="identity") +
  cp+
  scale_fill_manual(values=c("orange", "blue", "gray")) +
  facet_wrap(~Rank, scales = "free", nrow = 1) +
  theme_void() +
  theme(legend.position = "none") +
  geom_text(aes(label = Number_reads),
            position = position_stack(vjust = 0.5))

  

pie_deseq_ASV <- ggplot(deseq_for_pie_chart_ASV_all, aes(x="", y=Freq, fill=Var1)) +
...
deseq_for_pie_chart_reads <- deseq_for_pie_chart[,c("direction","Total_Reads_tbl")]
deseq_for_pie_chart_reads <- deseq_for_pie_chart_reads[!is.na(deseq_for_pie_chart_reads$Total_Reads_tbl),]
deseq_for_pie_chart_reads <- deseq_for_pie_chart_reads <- deseq_for_pie_chart_reads %>%                                        
  group_by(direction) %>% 
  summarise_at(vars(Total_Reads_tbl), 
               list(Number_reads = sum))
pie_deseq_reads <- ggplot(deseq_for_pie_chart_reads, aes(x="", y=Number_reads, fill=direction)) +
  geom_bar(stat="identity", width=1, color="white") +
  scale_fill_manual(values=c("orange", "blue", "gray")) +
...
pie_deseq_ASV_reads <- ggarrange(pie_deseq_ASV,pie_deseq_reads,nrow=1)
ggsave(pie_deseq_ASV_reads,filename = "/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/pie_deseq_ASV_reads.pdf", width = 9, height = 5)
...
project_data_2 <- subset_project_data %>%
  subset_samples(stunted == "Non-Stunted")
x <- parse_phyloseq(project_data_2)

sample_df <- as.data.frame(as.matrix(sample_data(project_data_2)))

# Convert counts to proportions
x$data$otu_table <- calc_obs_props(x, data = "otu_table", cols = sample_df$Sample)
#> Calculating proportions from counts for 20 columns for 1000 observations.

# Get per-taxon counts
x$data$tax_table <- calc_taxon_abund(x, data = "otu_table", cols = sample_df$Sample)
...
x$data$diff_table <- compare_groups(x, data = "tax_table",
                                    cols = sample_df$Sample,
                                    groups = sample_df$Cq_presence_absence)

x$data$diff_table$wilcox_p_value <- p.adjust(x$data$diff_table$wilcox_p_value,method = "BH")

range(x$data$diff_table$wilcox_p_value, finite = TRUE) 

x$data$diff_table$log2_median_ratio[x$data$diff_table$wilcox_p_value > 0.05] <- 0

pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Cladogram_presence_absence_non_stunted.pdf", height = 5, width = 7)
heat_tree_matrix(x,
                 data = "diff_table",
                 key_size = 0.67,
                 node_size = n_obs,
                 ...
                 node_color_range = c("blue","gray","orange"),
...
log_melt <- summary_table[,c("Cplt_taxo","Rank","Both_Country_Stunted","Both_Country_Non-Stunted")]

#Create a another column to indicate what is significant or not
...
head(log_melt3)
log_melt3_melt <- melt(log_melt3, id.vars=c("Cplt_taxo","order","Rank"),
...
pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/DESeq_summary_country_combined.pdf", height = 30, width = 10)
ggplot(log_melt3_mel, aes(x=variable, y=reorder(Cplt_taxo,-as.integer(factor(order)), FUN=min), size= value, color=direction)) +
  geom_point(alpha=0.5) +
  scale_color_manual(values=c("orange","blue"), name="Change in abundance") +
  theme(axis.text.x = element_text(angle = -45, hjust = 0)) +
...
pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/DESeq_summary_country_combined_allranks.pdf", height = 20, width = 19)
plot_deseq_sig_all <- ggplot(log_melt4_mel, aes(x=variable, y=reorder(Cplt_taxo,value_in), size= value, color=direction)) +
  geom_point(alpha=0.5) +
  theme_light() +
  scale_color_manual(values=c("orange","blue"), name="Change in abundance") +
...
log_melt4_mel <- log_melt4_mel[grep("Rank6",log_melt4_mel$Cplt_taxo),]


rank_name <- c("Rank3","Rank4","Rank5","Rank6")
for(i in 1:length(rank_name)){
  log_melt5_mel <- log_melt4_mel[grep(rank_name[i],log_melt4_mel$Cplt_taxo),]
  pdf(paste0("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/DESeq_summary_country_combined_similar_",rank_name[i],".pdf"), height = 20, width = 20)
  plot_deseq_sig_all <- ggplot(log_melt5_mel, aes(x=variable, y=reorder(Cplt_taxo2,value_in), size= value, color=direction)) +
    geom_point(alpha=0.5) +
    theme_light() +
    scale_color_manual(values=c("orange","blue"), name="Change in abundance") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size=20, face="bold"),
...
final_table$Sample <- factor(final_table$Sample, levels = c("Rank2","Rank3","Rank4","Rank5","Rank6"))
...
####################################################################
###            PLot with Spearman correlation test               ###
####################################################################

summary_correlation_table <- summary_correlation_table[,c("Cplt_taxo",
                                                          "Rho_Both_Country_Non-Stunted",
                                                          "Pvalue_Spearman_Both_Country_Non-Stunted",
                                                          "Rho_Both_Country_Stunted",
                                                          "Pvalue_Spearman_Both_Country_Stunted")]
summary_correlation_table <- summary_correlation_table[!is.na(summary_correlation_table$Cplt_taxo),]
summary_correlation_table_melt1 <- melt(summary_correlation_table, id.vars=c("Cplt_taxo",
                                                                            colnames(summary_correlation_table)[grep("Pvalue",colnames(summary_correlation_table))]),
                                       measure.vars = colnames(summary_correlation_table)[grep("Rho",colnames(summary_correlation_table))])[,c("Cplt_taxo","value","variable")];head(summary_correlation_table_melt1)
colnames(summary_correlation_table_melt1) <- c("Cplt_taxo","Rho","Stunted")
summary_correlation_table_melt1$Stunted <- gsub("Rho_Both_Country_","",summary_correlation_table_melt1$Stunted)

summary_correlation_table_melt2 <- melt(summary_correlation_table, id.vars=c("Cplt_taxo",
                                                                             colnames(summary_correlation_table)[grep("Rho",colnames(summary_correlation_table))]),
                                        measure.vars = colnames(summary_correlation_table)[grep("Pval",colnames(summary_correlation_table))])[,c("Cplt_taxo","value","variable")];head(summary_correlation_table_melt2)
colnames(summary_correlation_table_melt2) <- c("Cplt_taxo","Pvalue","Stunted")
summary_correlation_table_melt2$Stunted <- gsub("Pvalue_Spearman_Both_Country_","",summary_correlation_table_melt2$Stunted)

summary_correlation_table_melt <- merge(summary_correlation_table_melt1,summary_correlation_table_melt2,by=c("Cplt_taxo","Stunted"),all=FALSE)
#summary_correlation_table_melt$direction <- ifelse(summary_correlation_table_melt$Rho<0,"Increase","Decrease")


intersect(log_melt4_mel$Cplt_taxo,summary_correlation_table_melt$Cplt_taxo)
setdiff(log_melt4_mel$Cplt_taxo,summary_correlation_table_melt$Cplt_taxo)
log_melt44_mel <- unique(log_melt4_mel[,c("Cplt_taxo","Rank","value_in")])
log_melt5_mel <- merge(log_melt44_mel,summary_correlation_table_melt, by="Cplt_taxo",all=FALSE)
log_melt5_mel$direction <- ifelse(log_melt5_mel$Rho<0,"Increase","Decrease")
log_melt5_mel$Rho <- abs(log_melt5_mel$Rho)
log_melt5_mel$Rho_sig <- ifelse(log_melt5_mel$Pvalue<0.05,log_melt5_mel$Rho,NA)

pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Spearman_summary_country_combined_allranks.pdf", height = 20, width = 19)
plot_spearman_sig_all <- ggplot(log_melt5_mel, aes(x=Stunted, y=reorder(Cplt_taxo,value_in), size= Rho_sig, color=direction)) +
  geom_point(alpha=0.5) +
  theme_light() +
  scale_color_manual(values=c("orange","blue"), name="Change in abundance") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size=20, face="bold"),
...
summary_correlation_table_melt$Rho <- ifelse(summary_correlation_table_melt$Pvalue>0.05,NA,summary_correlation_table_melt$Rho)

pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Spearman_summary_country_combined.pdf", height = 50, width = 10)
ggplot(summary_correlation_table_melt, aes(x=Stunted, y=Cplt_taxo, size= Rho, color=direction)) +
  geom_point(alpha=0.5) +
  scale_color_manual(values=c("orange","blue"), name="Change in abundance") +
  theme(axis.text.x = element_text(angle = -45, hjust = 0)) #+
#facet_grid(rows = vars(Rank), scales = "free", space = "free")
...
intersect(summary_correlation_table_melt$Cplt_taxo,prop_table$Cplt_taxo)
setdiff(summary_correlation_table_melt$Cplt_taxo,prop_table$Cplt_taxo)
setdiff(prop_table$Cplt_taxo,summary_correlation_table_melt$Cplt_taxo)

test <- merge(summary_correlation_table_melt,prop_table,by="Cplt_taxo",all=TRUE)
pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Spearman_summary_country_combined_ranked.pdf", height = 50, width = 20)
plot_spearman <- ggplot(test, aes(x=Stunted, y=reorder(Cplt_taxo,order), size= Rho, color=direction)) +
  geom_point(alpha=0.5) +
  scale_color_manual(values=c("orange","blue"), name="Change in abundance") +
  theme(axis.text.x = element_text(angle = -45, hjust = 0)) +
  facet_grid(rows = vars(Rank), scales = "free", space = "free") +
...
pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Spearman_taxa_prop.pdf", height = 50, width = 30)
plot_taxa_proportion %>% insert_right(plot_spearman)
dev.off()
...
log_melt3_mel$Cplt_taxo <- gsub("[|]Rank.*","",log_melt3_mel$Cplt_taxo)
log_melt3_mel$Cplt_taxo <- gsub("Bacteria[|]","",log_melt3_mel$Cplt_taxo)
log_melt3_mel$Cplt_taxo <- gsub("Archaea[|]","",log_melt3_mel$Cplt_taxo)

prop_table <- prop_table[,-5]
intersect(prop_table$Cplt_taxo, log_melt3_mel$Cplt_taxo)
setdiff(prop_table$Cplt_taxo, log_melt3_mel$Cplt_taxo)
setdiff(log_melt3_mel$Cplt_taxo,prop_table$Cplt_taxo)


data <- merge(prop_table,log_melt3_mel, by="Cplt_taxo", all=TRUE)


final_table <- data.frame()
rank_vector <- c("Rank2","Rank3","Rank4","Rank5","Rank6")
for(rnk in 1:length(rank_vector)){
  test1_rnk <- subset(data, Rank==rank_vector[rnk])
  test <- as.data.frame(table(test1_rnk$order))
  test$Sample <- rank_vector[rnk]
  final_table <- rbind(final_table,test)
}
...
lefse_table$Cplt_taxo <- gsub("[|]Rank.*","",lefse_table$Cplt_taxo)

#Select only the ones that are significant
to_keep <- intersect(lefse_table$Cplt_taxo,log_melt4_mel$Cplt_taxo)
setdiff(log_melt4_mel$Cplt_taxo,lefse_table$Cplt_taxo)
full_abundance <- subset(lefse_table, Cplt_taxo %in% to_keep)
#full_abundance[full_abundance==1] <- 0 

#Melt
full_abundance_melt <- melt(full_abundance, id.vars=c("Cplt_taxo","Rank"));head(full_abundance_melt)


#Merge with metadata
qPCR_Metadata_sub <- qPCR_Metadata[,c("Sample","stunted","Cq_presence_absence","Cq")]
...
full_abundance_melt$Cq_presence_absence <- ifelse(full_abundance_melt$Cq_presence_absence=="Presence_qPCR","Presence Blastocystis","Absence Blastocystis")

order_info <- unique(log_melt4_mel[c("Cplt_taxo","value_in")])
full_abundance_melt <- merge(full_abundance_melt,order_info,by="Cplt_taxo")
#ggplot
pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/bubble_presence_absence_allranks.pdf", height = 10, width = 30)
bubble_plot <- ggplot(full_abundance_melt, aes(x=reorder(Sample,-Cq), y=reorder(Cplt_taxo,value_in), size=value2, color=Cq_presence_absence)) +
  geom_point() +
  scale_color_manual(values=c("gold","dodgerblue")) +
  facet_grid(Rank~stunted + Cq_presence_absence, scales = "free", space = "free") +
  theme_minimal() +
  theme(#axis.text.y = element_blank(),
        axis.title.y = element_blank(),
...
#
my_breaks = c(2, 10, 50, 250, 1250, 5000)
pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/heatmap_presence_absence_allranks.pdf", height = 10, width = 18)
heatmap_plot <- ggplot(full_abundance_melt, aes(x=reorder(Sample,-Cq), y=reorder(Cplt_taxo,value_in), fill=value2)) +
  geom_tile() +
...
pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/DESeq_and_heatmap.pdf", height = 10, width = 30)
plot_deseq_sig_all %>% insert_right(heatmap_plot, width = 5)
dev.off()

#
plot_combined <- ggarrange(plot_deseq_sig_all,bubble_plot,labels = c("A","B"), ncol = 2, nrow=1, align = "h");plot_combined
ggsave2(plot_combined, filename = "/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/DESeq_and_bubble2.pdf", height = 10, width = 27)


to_keep <- intersect(prop_table_melt$Cplt_taxo,full_abundance_melt$Cplt_taxo)
prop_table_melt <- subset(prop_table_melt, Cplt_taxo %in% to_keep)
order_info <- unique(log_melt4_mel[c("Cplt_taxo","value_in")])
prop_table_melt <- uniqmerge(prop_table_melt,order_info,by="Cplt_taxo")
prop_table_melt$value <- prop_table_melt$value/2
pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Proportion_Prevalence_SigDeseq.pdf", height = 10, width = 7)
plot3 <- ggplot(prop_table_melt, aes(y=reorder(Cplt_taxo,value_in), x=value)) +
  geom_bar(stat="identity") +
  theme_light() +
  facet_grid(Rank ~ variable, scales = "free_y", space = "free_y") +
  theme(axis.text.y = element_blank(),
        axis.title.y = element_blank(),
...
plot_combined <- ggarrange(plot_deseq_sig_all,bubble_plot,plot3, ncol = 3, nrow=1, widths = c(0.4, 0.4,0.1));plot_combined
ggsave2(plot_combined, filename = "/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/DESeq_bubble2_Proportion_Prevalence.pdf", height = 10, width = 40)


####################################################################

####################################################################
### Relative abundance of reads per samples
####################################################################

#Select only the ones that are significant
to_keep <- intersect(rel_abun_tbl_rnk_melt_all$Cplt_taxo,log_melt4_mel$Cplt_taxo)
setdiff(log_melt4_mel$Cplt_taxo,rel_abun_tbl_rnk_melt_all$Cplt_taxo)
full_abundance <- subset(rel_abun_tbl_rnk_melt_all, Cplt_taxo %in% to_keep)
#full_abundance[full_abundance==1] <- 0 

#Merge with metadata
order_info <- unique(log_melt4_mel[c("Cplt_taxo","value_in")])
full_abundance_melt <- merge(full_abundance,order_info,by="Cplt_taxo")

#Merge with metadata
qPCR_Metadata_sub <- qPCR_Metadata[,c("Sample","stunted","Cq_presence_absence")]
full_abundance_melt <- merge(qPCR_Metadata_sub, full_abundance_melt, by.x ="Sample", by.y = "variable", all=FALSE)

pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Jitter_rel_abun_allrank_sorted.pdf", height = 8, width = 12)
plot_taxa_proportion <- ggplot(full_abundance_melt, aes(y=reorder(Cplt_taxo,value_in), x=value, color=Cq_presence_absence)) +
  geom_point(size=0.2) +
  theme_light() +
...
####################################################################
###           Scatter plot Prevotella and Bacteroides            ###
####################################################################

tax = as.data.frame(tax_table(subset_project_data))
to_keep <- tax[grepl("revotella|Bacteroides|Succinivibrio", tax$Rank6),]

otu = as.data.frame(t(otu_table(subset_project_data)))

tax_otu <- merge(to_keep, otu, by="row.names")[,c("Rank5", colnames(otu))]
tax_otu_agg <- as.data.frame(tax_otu %>%
  group_by(Rank5) %>% 
  summarise_each(funs(sum)))
row.names(tax_otu_agg) <- tax_otu_agg$Rank5
tax_otu_agg$Rank5 <- NULL
tax_otu_agg <- as.data.frame(t(tax_otu_agg))

sample_df <- as.data.frame(as.matrix(sample_data(subset_project_data)))

merge_spl_tax_otu <- merge(sample_df,tax_otu_agg, by="row.names")

merge_spl_tax_otu <- subset(merge_spl_tax_otu, Bacteroidaceae>0)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Prevotellaceae>0)
merge_spl_tax_otu <- subset(merge_spl_tax_otu, Succinivibrionaceae>0)

merge_spl_tax_otu$Cq <- as.numeric(merge_spl_tax_otu$Cq)
ggplot(merge_spl_tax_otu, aes(x=Bacteroidaceae, y=Prevotellaceae)) +
  geom_point(aes(color = Cq_presence_absence)) +
  scale_x_continuous(trans='log10') +
  scale_y_continuous(trans='log10') +
  stat_cor(method="spearman")  +
  ylab("Prevotellaceae (nb reads)") +
  xlab("Bacteroidaceae (nb reads)") +
  theme_classic()


shapiro.test(log(merge_spl_tax_otu$Bacteroidaceae,10))
shapiro.test(log(merge_spl_tax_otu$Prevotellaceae,10))
hist(log(merge_spl_tax_otu$Bacteroidaceae,10))
hist(log(merge_spl_tax_otu$Prevotellaceae,10))

cor.test(log(merge_spl_tax_otu$Bacteroidaceae,10),log(merge_spl_tax_otu$Prevotellaceae,10), method = "spearman")
cor.test(merge_spl_tax_otu$Bacteroidaceae,merge_spl_tax_otu$Prevotellaceae, method = "spearman")
cor.test(log(merge_spl_tax_otu$Bacteroidaceae,10),log(merge_spl_tax_otu$Prevotellaceae,10), method = "pearson")
cor.test(merge_spl_tax_otu$Bacteroidaceae,merge_spl_tax_otu$Prevotellaceae, method = "pearson")



#merge_spl_tax_otu <- subset(merge_spl_tax_otu, Cq_presence_absence =="Presence_qPCR")
merge_spl_tax_otu$Cq_presence_absence_mod <- ifelse(merge_spl_tax_otu$Cq_presence_absence=="Presence_qPCR","Presence Blastocystis","Absence Blastocystis")
merge_spl_tax_otu$Cq_mod <- ifelse(merge_spl_tax_otu$Cq==0,41,merge_spl_tax_otu$Cq)
plt_prev_abu <- ggplot(merge_spl_tax_otu, aes(x=Cq_mod, y=Prevotellaceae)) +
  geom_point(aes(color = Cq_presence_absence_mod)) +
  scale_color_manual(values=c("orange","blue")) +
  scale_y_continuous(trans='log10') +
  stat_cor(method="spearman")  +
  ylab("Prevotellaceae (nb reads)") +
  xlab("Blastocystis (Ct values)") +
  scale_x_reverse() +
  theme_classic() +
  facet_wrap(~Cq_presence_absence_mod, ncol=2, scale="free_x") +
  theme(legend.position = "none",
        axis.title.x = element_blank(),
        strip.text.x = element_blank())


plt_bact_abu <- ggplot(merge_spl_tax_otu, aes(x=Cq_mod, y=Bacteroidaceae)) +
  geom_point(aes(color = Cq_presence_absence_mod)) +
  scale_color_manual(values=c("orange","blue")) +
  scale_y_continuous(trans='log10') +
  stat_cor(method="spearman")  +
  ylab("Bacteroidaceae (nb reads)") +
  xlab("Blastocystis (Ct values)") +
  scale_x_reverse() +
  theme_classic() +
  facet_wrap(~Cq_presence_absence_mod, ncol=2, scale="free_x") +
...
plt_bact_abu <- ggplot(merge_spl_tax_otu, aes(x=Cq_mod, y=Succinivibrionaceae)) +
  geom_point(aes(color = Cq_presence_absence_mod)) +
  scale_color_manual(values=c("orange","blue")) +
  scale_y_continuous(trans='log10') +
  stat_cor(method="spearman")  +
...
cor.test(log(merge_spl_tax_otu$Cq,10),log(merge_spl_tax_otu$Bacteroidaceae,10), method = "spearman")
cor.test(merge_spl_tax_otu$Cq,merge_spl_tax_otu$Bacteroidaceae, method = "spearman")
cor.test(log(merge_spl_tax_otu$Cq,10),log(merge_spl_tax_otu$Bacteroidaceae,10), method = "pearson")
cor.test(merge_spl_tax_otu$Cq,merge_spl_tax_otu$Bacteroidaceae, method = "pearson")
shapiro.test(log(merge_spl_tax_otu$Bacteroidaceae,10))
shapiro.test(log(merge_spl_tax_otu$Cq,10))
hist(log(merge_spl_tax_otu$Bacteroidaceae,10))
hist(log(merge_spl_tax_otu$Cq,10))

cor.test(log(merge_spl_tax_otu$Cq,10),log(merge_spl_tax_otu$Prevotellaceae,10), method = "spearman")
cor.test(merge_spl_tax_otu$Cq,merge_spl_tax_otu$Prevotellaceae, method = "spearman")
cor.test(log(merge_spl_tax_otu$Cq,10),log(merge_spl_tax_otu$Prevotellaceae,10), method = "pearson")
cor.test(merge_spl_tax_otu$Cq,merge_spl_tax_otu$Prevotellaceae, method = "pearson")
shapiro.test(log(merge_spl_tax_otu$Prevotellaceae,10))
shapiro.test(log(merge_spl_tax_otu$Cq,10))
hist(log(merge_spl_tax_otu$Prevotellaceae,10))
hist(log(merge_spl_tax_otu$Cq,10))
...
#Rank6
tax_otu <- merge(to_keep, otu, by="row.names")[,c("Rank6", colnames(otu))]
tax_otu_agg <- as.data.frame(tax_otu %>%
                               group_by(Rank6) %>% 
                               summarise_each(funs(sum)))
row.names(tax_otu_agg) <- tax_otu_agg$Rank6
tax_otu_agg$Rank6 <- NULL
tax_otu_agg <- as.data.frame(t(tax_otu_agg))

merge_spl_tax_otu <- merge(sample_df,tax_otu_agg, by="row.names")
merge_spl_tax_otu <- merge(merge_spl_tax_otu,diet_score,by="Sample")


merge_spl_tax_otu$Cq <- as.numeric(merge_spl_tax_otu$Cq)
#merge_spl_tax_otu <- subset(merge_spl_tax_otu, Cq_presence_absence =="Presence_qPCR")

lm <- lm(Prevotella_log~Cq+diet_score,data=merge_spl_tax_otu)
summary(lm)

#merge_spl_tax_otu_melt <- melt(merge_spl_tax_otu, id.vars=c("Cq","diet_score"),measure.vars = c("Alloprevotella","Bacteroides","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un"))#[, c("binome","value")]

merge_spl_tax_otu <- merge_spl_tax_otu[,c("Cq","Cq_presence_absence","Alloprevotella","Bacteroides","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")]
merge_spl_tax_otu$Cq_presence_absence_mod <- ifelse(merge_spl_tax_otu$Cq_presence_absence=="Presence_qPCR","Presence Blastocystis","Absence Blastocystis")
merge_spl_tax_otu$Cq_mod <- ifelse(merge_spl_tax_otu$Cq==0,41,merge_spl_tax_otu$Cq)
bacterial_taxa <- c("Alloprevotella","Bacteroides","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")
list_scatterplot <- list()
for(i in 1:length(bacterial_taxa)) {
  interm_table <- merge_spl_tax_otu[,c("Cq_mod","Cq_presence_absence_mod",bacterial_taxa[i])]
  colnames(interm_table) <- c("Cq_mod","Cq_presence_absence_mod","Bacterial_Taxa")
  interm_table <- subset(interm_table, Bacterial_Taxa>0)
  plot <- ggplot(interm_table, aes(x=Cq_mod, y= Bacterial_Taxa)) +
    geom_point(aes(color = Cq_presence_absence_mod)) +
    scale_color_manual(values=c("orange","blue")) +
    scale_x_continuous(trans='log10') +
    scale_y_continuous(trans='log10') +
    stat_cor(method="spearman") +
    ylab(gsub("_"," ",bacterial_taxa[i])) +
    scale_x_reverse() +
    theme_classic() +
    facet_wrap(~Cq_presence_absence_mod, ncol=2, scale="free_x") +
    theme(legend.position = "none",
          axis.title.x = element_blank(),
          strip.text.x = element_blank())
  list_scatterplot[[length(list_scatterplot)+1]] <- plot
}
...
plot <- ggarrange(list_scatterplot[[2]],list_scatterplot[[4]],list_scatterplot[[8]], labels = c("A","B","C"), nrow = 3, ncol = 1)
ggsave(plot,filename = "/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/scatterplot_Cq_prevo_bacteroides.pdf", width = 12, height = 3)

merge_spl_tax_otu_mel <- merge_spl_tax_otu[,c("Cq","Cq_presence_absence_mod","Alloprevotella","Bacteroides","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")]
merge_spl_tax_otu_mel <- melt(merge_spl_tax_otu_mel, id.vars=c("Cq","Cq_presence_absence_mod"),measure.vars = c("Alloprevotella","Bacteroides","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un"))
merge_spl_tax_otu_mel$Cq_mod <- ifelse(merge_spl_tax_otu_mel$Cq==0,41,merge_spl_tax_otu_mel$Cq)
merge_spl_tax_otu_mel <- subset(merge_spl_tax_otu_mel, Cq_presence_absence_mod=="Presence Blastocystis")
ggplot(merge_spl_tax_otu_mel, aes(x = Cq_mod, y = value, color = variable)) +
  geom_point() + 
  scale_y_continuous(trans='log10') +
...
##Cq value and diversity of Prevotella.
to_keep <- tax[grepl("revotella", tax$Rank6),]

otu = as.data.frame(t(otu_table(subset_project_data)))

tax_otu <- merge(to_keep, otu, by="row.names")[,c("Rank7", colnames(otu))]
tax_otu_agg <- as.data.frame(tax_otu %>%
                               group_by(Rank7) %>% 
                               summarise_each(funs(sum)))
row.names(tax_otu_agg) <- tax_otu_agg$Rank7
tax_otu_agg$Rank7 <- NULL
tax_otu_agg <- as.data.frame(t(tax_otu_agg))

sample_df <- as.data.frame(as.matrix(sample_data(subset_project_data)))

merge_spl_tax_otu <- merge(sample_df[,c("Cq","Cq_presence_absence")],tax_otu_agg, by="row.names")

merge_spl_tax_otu$Cq <- as.numeric(merge_spl_tax_otu$Cq)
merge_spl_tax_otu$Cq_presence_absence_mod <- ifelse(merge_spl_tax_otu$Cq_presence_absence=="Presence_qPCR","Presence Blastocystis","Absence Blastocystis")
merge_spl_tax_otu$Cq_mod <- ifelse(merge_spl_tax_otu$Cq==0,41,merge_spl_tax_otu$Cq)
merge_spl_tax_otu[,-c(1:3)][merge_spl_tax_otu[,c("Alloprevotella","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")] > 0] <- 1
merge_spl_tax_otu$richness <- rowSums(merge_spl_tax_otu[,c("Alloprevotella","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")])
#merge_spl_tax_otu <- subset(merge_spl_tax_otu, Cq>0)

plt_prev_rich <- ggplot(merge_spl_tax_otu, aes(x=Cq_mod, y= richness)) +
  geom_point(aes(color = Cq_presence_absence_mod), size=0.1) +
  scale_color_manual(values=c("orange","blue")) +
                  facet_wrap(~Cq_presence_absence_mod, ncol=2, scale="free_x") +
                  stat_cor(method="spearman") +
                  ylab("Prevotella genus richness") +
                  xlab("Blastocystis abundance (Ct values)") +
                  scale_x_reverse() +
                  theme_classic() +
                  theme(legend.position = "none",
                        axis.title.x = element_blank(),
...
plot <- ggarrange(plt_prev_abu,plt_bact_abu,list_scatterplot[[2]],list_scatterplot[[4]],list_scatterplot[[8]],plt_prev_rich, labels = c("A","B","C"), nrow = 6, ncol = 1)
ggsave(plot,filename = "/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/summary_scatterplot_Cq_prevo_bacteroides.pdf", width = 9, height = 13)


####################################################################

####################################################################
###     Diet score Scatter plot Prevotella and Bacteroides       ###
####################################################################
...
merge_spl_tax_otu <- merge_spl_tax_otu[,c("diet_score.y","Cq_presence_absence",colnames(tax_otu_agg))]
merge_spl_tax_otu$Cq_presence_absence_mod <- ifelse(merge_spl_tax_otu$Cq_presence_absence=="Presence_qPCR","Presence Blastocystis","Absence Blastocystis")
bacterial_taxa <- colnames(tax_otu_agg)
list_scatterplot <- list()
for(i in 1:length(bacterial_taxa)) {
  interm_table <- merge_spl_tax_otu[,c("diet_score.y","Cq_presence_absence_mod",bacterial_taxa[i])]
  colnames(interm_table) <- c("diet_score","Cq_presence_absence_mod","Bacterial_Taxa")
  interm_table <- subset(interm_table, Bacterial_Taxa>0)
  plot <- ggplot(interm_table, aes(x=diet_score, y= Bacterial_Taxa)) +
    geom_point(aes(color = Cq_presence_absence_mod)) +
    scale_color_manual(values=c("orange","blue")) +
    scale_y_continuous(trans='log10') +
    stat_cor(method="spearman") +
...
plot1 <-list_scatterplot[[1]]#Neg slope for Alloprevotella
plot2 <-list_scatterplot[[2]]#Positive relationship with bacteroides in the presence of blastocystis
...
plot3 <-list_scatterplot[[1]]
plot4 <-list_scatterplot[[2]]

plot1 <- ggarrange(plot3,plot4,plot1,plot2, nrow = 4, ncol = 1)

ggsave(plot1,filename = "/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/scatterplot_diet_prevo_bacteroides_family.pdf", width = 7, height = 10)


#Rank6
to_keep <- tax[grepl("revotella|Bacteroides", tax$Rank6),]
tax_otu <- merge(to_keep, otu, by="row.names")[,c("Rank6", colnames(otu))]
tax_otu_agg <- as.data.frame(tax_otu %>%
                               group_by(Rank6) %>% 
                               summarise_each(funs(sum)))
...
merge_spl_tax_otu <- merge_spl_tax_otu[,c("diet_score.y","Cq_presence_absence","Alloprevotella","Bacteroides","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")]
merge_spl_tax_otu$Cq_presence_absence_mod <- ifelse(merge_spl_tax_otu$Cq_presence_absence=="Presence_qPCR","Presence Blastocystis","Absence Blastocystis")
bacterial_taxa <- c("Alloprevotella","Bacteroides","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")
list_scatterplot <- list()
for(i in 1:length(bacterial_taxa)) {
...
list_scatterplot[[1]]#Neg slope for Alloprevotella
list_scatterplot[[2]]#Positive relationship with bacteroides in the presence of blastocystis
list_scatterplot[[3]]
list_scatterplot[[4]]
list_scatterplot[[5]]#Neg slope for Prevotella
list_scatterplot[[6]]
list_scatterplot[[7]]#Neg slope for Prevotella 7
list_scatterplot[[8]]#Neg slope for Prevotella 9
list_scatterplot[[9]]#Neg slope for Prevotella NK3B31
list_scatterplot[[10]]

plot <- ggarrange(list_scatterplot[[1]],list_scatterplot[[2]],list_scatterplot[[3]],
                  list_scatterplot[[4]],list_scatterplot[[5]],list_scatterplot[[6]],
                  list_scatterplot[[7]],list_scatterplot[[8]],list_scatterplot[[9]],
                  list_scatterplot[[10]], nrow = 10, ncol = 1)
ggsave(plot,filename = "/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/scatterplot_diet_prevo_bacteroides.pdf", width = 10, height = 20)
...
plot <- ggarrange(list_scatterplot[[1]],list_scatterplot[[2]],list_scatterplot[[3]],
                  ...
                  list_scatterplot[[7]],list_scatterplot[[8]],list_scatterplot[[9]],
                  list_scatterplot[[10]], nrow = 10, ncol = 1)
ggsave(plot,filename = "/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/scatterplot_diet_prevo_bacteroides_genus.pdf", width = 10, height = 20)
...
merge_spl_tax_otu <- merge_spl_tax_otu[,c("diet_score.y","Cq_presence_absence","Alloprevotella","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")]
merge_spl_tax_otu$Cq_presence_absence_mod <- ifelse(merge_spl_tax_otu$Cq_presence_absence=="Presence_qPCR","Presence Blastocystis","Absence Blastocystis")
merge_spl_tax_otu[,c("Alloprevotella","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")][merge_spl_tax_otu[,c("Alloprevotella","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")] > 0] <- 1
merge_spl_tax_otu$richness <- rowSums(merge_spl_tax_otu[,c("Alloprevotella","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")])
#merge_spl_tax_otu <- subset(merge_spl_tax_otu, Cq>0)

plt_prev_rich <- ggplot(merge_spl_tax_otu, aes(x=diet_score.y, y= richness)) +
  geom_point(aes(color = Cq_presence_absence_mod)) +
  scale_color_manual(values=c("orange","blue")) +
  facet_wrap(~Cq_presence_absence_mod, ncol=2, scale="free_x") +
  stat_cor(method="spearman") +
...
plt_prev_rich_all <- ggplot(merge_spl_tax_otu, aes(x=diet_score.y, y= richness)) +
  geom_point() +
  stat_cor(method="spearman") +
  ylab("Prevotella genus richness") +
  theme_classic() +
...
plot <- ggarrange(plt_prev_rich,plt_prev_rich_all, nrow = 2, ncol = 1)
ggsave(plot,filename = "/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/scatterplot_diet_prevo_richness.pdf", width = 7, height = 7)


####################################################################

####################################################################
### Develop model prevatola, blasto, diet
####################################################################

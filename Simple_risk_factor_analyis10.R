...
#log_melt_rnk2 <- log_melt[grep('Rank5',log_melt$Cplt_taxo),]
#log_melt_rnk3$Cplt_taxo <- gsub("ASV.*","",log_melt_rnk3$Cplt_taxo)
...
pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/DESeq_summary_country_combined.pdf", height = 30, width = 20)
ggplot(log_melt3_mel, aes(x=variable, y=reorder(Cplt_taxo,-as.integer(factor(order)), FUN=min), size= value, color=direction)) +
  geom_point(alpha=0.5) +
  scale_color_manual(values=c("orange","blue"), name="Change in abundance") +
  theme(axis.text.x = element_text(angle = -45, hjust = 0))

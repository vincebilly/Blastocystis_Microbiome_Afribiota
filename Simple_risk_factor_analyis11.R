...
for(p in 1:length(pays_vector)){
  ...
  for(s in 1:length(stunted_vector)){
    ...
    for(rnk in 1:length(rank_vector)){
      ...
      res_p_ordered_wtaxa.var3$Cplt_taxo <- paste0(res_p_ordered_wtaxa.var3$Rank2,"|",
...
####################################################################
###         Create the table with all taxa combinaison           ###
####################################################################
...
lefse_table$Cplt_taxo <- paste0(lefse_table$Rank2,"|",lefse_table$Rank3,"|",lefse_table$Rank4,"|",lefse_table$Rank5,"|",lefse_table$Rank6,"|",lefse_table$Rank)
lefse_table$Cplt_taxo <- gsub("[|]NA","",lefse_table$Cplt_taxo)
lefse_table <- lefse_table[,!grepl("Rank", colnames(lefse_table))]
lefse_table <- lefse_table[,!grepl("Row.names", colnames(lefse_table))]

prop_table <- data.frame()                                                     #Create dataframe
rank_vector <- c("Rank2","Rank3","Rank4","Rank5","Rank6")
for(rnk in 1:length(rank_vector)){
  test <- lefse_table[grep(rank_vector[rnk],lefse_table$Cplt_taxo),]
  ...
  colnames(tbl_plt) <- c("Cplt_taxo","Proportion","Prevalence")
...
prop_table_melt <- melt(prop_table, id.vars=c("Cplt_taxo","Rank","order"),
                       measure.vars = c("Proportion","Prevalence"));head(prop_table_melt)

pdf("/Users/vincebilly/Desktop/vince/phd/statistic/afribiota_vince/Simple_risk_factor/figure/Taxa_proportion.pdf", height = 35, width = 10)
plot1 <- ggplot(prop_table_melt, aes(y=reorder(Cplt_taxo,order), x=value)) +
...
full_abundance <- full_abundance[,-c(1:8)]
full_abundance$Cplt_taxo <- gsub("\\|NA","",full_abundance$Cplt_taxo)

#Select only the ones that are significant
to_keep <- intersect(full_abundance$Cplt_taxo,log_melt4_mel$Cplt_taxo2)
setdiff(log_melt4_mel$Cplt_taxo2,full_abundance$Cplt_taxo)
full_abundance <- subset(full_abundance, Cplt_taxo %in% to_keep)
#full_abundance[full_abundance==1] <- 0 

#Melt
full_abundance_melt <- melt(full_abundance, id.vars=c("Cplt_taxo"));head(full_abundance_melt)
...
#ggplot
bubble_plot <- ggplot(full_abundance_melt, aes(x=reorder(Sample,-Cq), y=Cplt_taxo, size=value2, color=Cq_presence_absence)) +
  geom_point() +
...
#
full_abundance_melt$
heatmap_plot <- ggplot(full_abundance_melt, aes(x=reorder(Sample,Cq), y=Cplt_taxo, fill=value2)) +
  geom_tile() +
...
merge_spl_tax_otu <- merge_spl_tax_otu[,c("Cq","Cq_presence_absence","Alloprevotella","Paraprevotella","Prevotella","Prevotella_2","Prevotella_6","Prevotella_7","Prevotella_9","Prevotellaceae_NK3B31_group","Prevotellaceae_UCG-003","Prevotellaceae_Un")]
merge_spl_tax_otu$Cq <- as.numeric(merge_spl_tax_otu$Cq)
merge_spl_tax_otu$Cq_presence_absence_mod <- ifelse(merge_spl_tax_otu$Cq_presence_absence=="Presence_qPCR","Presence Blastocystis","Absence Blastocystis")
merge_spl_tax_otu$Cq_mod <- ifelse(merge_spl_tax_otu$Cq==0,41,merge_spl_tax_otu$Cq)
